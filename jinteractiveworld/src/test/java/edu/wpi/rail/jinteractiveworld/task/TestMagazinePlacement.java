...
public class TestMagazinePlacement {

	@Test
	public void testConstructor() throws SecurityException,
			NoSuchFieldException, IllegalArgumentException,
			IllegalAccessException {
		Field f1 = MagazinePlacement.class.getDeclaredField("NAME");
		f1.setAccessible(true);
		Field f2 = MagazinePlacement.class.getDeclaredField("ITEMS");
		f2.setAccessible(true);

		RelevanceTable table = new GroundTruthTable();
		HashSet<Model> models = new HashSet<Model>();
		models.add(new ClusteringModel(new DataSet(), new Fork(), new Spoon()));
		MagazinePlacement ts = new MagazinePlacement(table, models);
